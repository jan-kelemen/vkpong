...
// Functions
static char const* ImGui_ImplGlfw_GetClipboardText(void* user_data)
{
    return glfwGetClipboardString((GLFWwindow*) user_data);
}

static void ImGui_ImplGlfw_SetClipboardText(void* user_data, char const* text)
{
...
static int ImGui_ImplGlfw_TranslateUntranslatedKey(int key, int scancode)
{
    ...
    char const* key_name = glfwGetKeyName(key, scancode);
    ...
    {
        char const char_names[] = "`-=[]\\,;\'./";
        int const char_keys[] = {GLFW_KEY_GRAVE_ACCENT,
            GLFW_KEY_MINUS,
            GLFW_KEY_EQUAL,
        ...
        else if (char const* p = strchr(char_names, key_name[0]))
        {
    ...
    int mods)
{
    ...
    io.SetKeyEventNativeData(imgui_key,
        keycode,
...
#ifdef __EMSCRIPTEN__
static EM_BOOL
ImGui_ImplEmscripten_WheelCallback(int, EmscriptenWheelEvent const* ev, void*)
{
...
#ifdef _WIN32
    bd->GlfwWndProc =
        (WNDPROC)::GetWindowLongPtrW((HWND) main_viewport->PlatformHandleRaw,
            GWLP_WNDPROC);
    IM_ASSERT(bd->GlfwWndProc != nullptr);
    ::SetWindowLongPtrW((HWND) main_viewport->PlatformHandleRaw,
        GWLP_WNDPROC,
...
#ifdef __EMSCRIPTEN__
        bool const is_window_focused = true;
#else
        bool const is_window_focused =
        ...
        if (is_window_focused)
        {
            ...
            if (bd->MouseWindow == nullptr)
            {
                ...
                bd->LastValidMousePos =
                    ImVec2((float) mouse_x, (float) mouse_y);
...
void ImGui_ImplGlfw_NewFrame()
{
    ...
    if (w > 0 && h > 0)
        io.DisplayFramebufferScale = ImVec2((float) display_w / (float) w,
    ...
    void* user_data)
{
    ...
    emscripten_get_element_css_size(bd->CanvasSelector,
        &canvas_width,
        &canvas_height);
