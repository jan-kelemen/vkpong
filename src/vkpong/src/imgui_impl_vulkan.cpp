        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                if (sizeof(ImTextureID) < sizeof(ImU64))
                {
                    ...
                    IM_ASSERT(
                        pcmd->TextureId == (ImTextureID) bd->FontDescriptorSet);
...
bool ImGui_ImplVulkan_CreateFontsTexture()
{
    ...
    bd->FontDescriptorSet =
        (VkDescriptorSet) ImGui_ImplVulkan_AddTexture(bd->FontSampler,
            bd->FontView,
...
void ImGui_ImplVulkan_DestroyDeviceObjects()
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    ImGui_ImplVulkan_DestroyWindowRenderBuffers(v->Device,
...
bool ImGui_ImplVulkan_LoadFunctions(
    PFN_vkVoidFunction (
        *loader_func)(char const* function_name, void* user_data),
    void* user_data)
{
    ...
    VkAllocationCallbacks const* allocator)
{
    for (uint32_t n = 0; n < buffers->Count; n++)
        ImGui_ImplVulkan_DestroyFrameRenderBuffers(device,
...
VkSurfaceFormatKHR ImGui_ImplVulkanH_SelectSurfaceFormat(
    ...
    VkFormat const* request_formats,
...
VkPresentModeKHR ImGui_ImplVulkanH_SelectPresentMode(
    VkPhysicalDevice physical_device,
    VkSurfaceKHR surface,
    VkPresentModeKHR const* request_modes,
    ...
    uint32_t min_image_count)
{
    ...
    for (uint32_t i = 0; i < wd->SemaphoreCount; i++)
        ImGui_ImplVulkanH_DestroyFrameSemaphores(device,
    ...
    // Create Swapchain
    {
        ...
        check_vk_result(err);
        err = vkGetSwapchainImagesKHR(device,
            wd->Swapchain,
            &wd->ImageCount,
            nullptr);
        check_vk_result(err);
    ...
    // Create The Image Views
    {
        ...
        VkImageSubresourceRange image_range = {VK_IMAGE_ASPECT_COLOR_BIT,
            0,
            1,
            0,
            1};
